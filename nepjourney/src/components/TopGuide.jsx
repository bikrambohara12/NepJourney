    ...
    <div className="flex flex-col items-center gap-6 my-20 text-gray-900 md:mx-10">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 pt-6 px-4 sm:px-0">
        {guides && guides.length > 0 ? (
          ...
          <p className="text-red-500 text-center">No guides found.</p>
        )}
      </div>

      <button
        onClick={() => navigate('/guides')} className="bg-blue-100 hover:bg-blue-200 text-gray-700 px-16 py-4 text-lg rounded-full mt-12 transition-colors duration-300">
        More
      </button>
    </div>
  );
